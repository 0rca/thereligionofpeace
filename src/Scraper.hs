...
extractData =
    map (normaliseColumns . extractColumns) . extractRows . extractTable . extractTags
...
extractColumns =
    map (map (unwords . words . fromTagText) . filter isTagText) . partitions (~== td_)
...
readDate = timeParse
    [Format_Year4, Format_Text '.', Format_Month2, Format_Text '.', Format_Day2]
